...
/**
 * Bit-cost tables and residual-cost estimation for VP8 rate-distortion mode selection.
 * <p>
 * Ported from libwebp ({@code src/enc/cost.c} + {@code src/enc/cost_enc.c} + {@code src/enc/cost_enc.h}).
...
final class VP8Costs {
    ...
    /**
     * Cost of coding a single bit at probability {@code proba} in {@code 1/256} bit units.
     * libwebp {@code VP8EntropyCost[256]}.
     */
    // @formatter:off
    static final int[] ENTROPY_COST = {
    ...
    /**
     * Fixed part of the coefficient-level cost, derived from the token-tree structure
     * independently of the probability state. libwebp {@code VP8LevelFixedCosts[MAX_LEVEL+1]}.
     */
    // @formatter:off
    static final int[] LEVEL_FIXED_COSTS = {
    ...
    /**
     * For each variable level {@code 1..67}, the pattern of internal token-tree
     * nodes (in {@code [0]}) and the bit values taken through them (in {@code [1]}).
     * libwebp {@code VP8LevelCodes[MAX_VARIABLE_LEVEL][2]}.
     */
    // @formatter:off
    static final int[][] LEVEL_CODES = {
    ...
    /**
     * Fixed bit-cost per chroma 8x8 mode (includes the {@code VP8BitCost(1, 145)}
     * macroblock-mode selector). libwebp {@code VP8FixedCostsUV[4]}, indexed by
     * {@code DC_PRED, V_PRED, H_PRED, TM_PRED}.
     */
    static final int[] FIXED_COSTS_UV = { 302, 984, 439, 642 };

    /**
     * Fixed bit-cost per 16x16 luma mode (includes the {@code VP8BitCost(0, 145)}
     * macroblock-mode selector). libwebp {@code VP8FixedCostsI16[4]}, indexed by
     * {@code DC_PRED, V_PRED, H_PRED, TM_PRED}.
     */
    static final int[] FIXED_COSTS_I16 = { 663, 919, 872, 919 };

    /**
     * Fixed bit-cost per 4x4 luma sub-block mode, keyed by
     * {@code [aboveMode][leftMode][subBlockMode]}. libwebp
     * {@code VP8FixedCostsI4[NUM_BMODES][NUM_BMODES][NUM_BMODES]}.
     */
    // @formatter:off
    static final int[][][] FIXED_COSTS_I4 = {
    ...
    /**
     * Cost in {@code 1/256} bits of coding {@code bit} (0 or 1) at probability {@code proba}.
     * Mirrors libwebp's {@code VP8BitCost}.
     */
    static int bitCost(int bit, int proba) {
        return bit == 0 ? ENTROPY_COST[proba] : ENTROPY_COST[255 - proba];
    }
    ...
    /**
     * Cost of the variable-length portion of a coefficient level ({@code level in 1..67}),
     * walking the level-code pattern for the given probability row. Mirrors libwebp's
     * {@code VariableLevelCost}.
     */
    private static int variableLevelCost(int level, int @NotNull [] probs) {
        int pattern = LEVEL_CODES[level - 1][0];
        int bits = LEVEL_CODES[level - 1][1];
        int cost = 0;
        for (int i = 2; pattern != 0; i++) {
            if ((pattern & 1) != 0)
                cost += bitCost(bits & 1, probs[i]);
            bits >>>= 1;
            pattern >>>= 1;
        }
        return cost;
     ...
     *                  {@code TYPE_I4_AC}
     * @param first first coefficient index to cost (0 for most types, 1 for I16 AC)
